...
const RelatedPackagesStrip = ({
  ...
  const looped = packages.length >= 3 ? [...packages, ...packages] : packages;
  ...
  return (
    ...
    >
      <div className={isDark ? "container mx-auto px-4 max-w-[1400px]" : "px-4 sm:px-6 md:px-8"}>
        <div className="mb-8">
          <span className={`text-xs font-semibold uppercase tracking-widest ${isDark ? "text-amber-400" : "text-primary"}`}>
            Bundle & Save
          </span>
          <h2
        ...
        >
          {looped.map((pkg, i) => (
            <motion.div
              ...
              transition={{ delay: (i % packages.length) * 0.08, duration: 0.5 }}
              ...
              >
                <div className="aspect-[16/10] overflow-hidden relative">
                  ...
                  <span className={`absolute top-3 left-3 text-[10px] font-bold px-2.5 py-1 rounded-full ${
                    isDark ? "bg-amber-400 text-black" : "gradient-gold text-primary-foreground"
                  }`}>
                    {Math.round(((pkg.originalPrice - pkg.price) / pkg.originalPrice) * 100)}% OFF
                  </span>
                </div>
                <div className="p-4">
                  <h3 className={`font-display font-bold text-base mb-2 line-clamp-1 ${isDark ? "text-white" : "text-foreground"}`}>
                    {pkg.name}
                  </h3>
                  <div className={`flex items-center gap-3 text-xs mb-3 ${isDark ? "text-white/60" : "text-muted-foreground"}`}>
                    <span className="flex items-center gap-1"><MapPin size={11} /> {pkg.location}</span>
                    <span className="flex items-center gap-1"><Clock size={11} /> {pkg.duration}</span>
                  </div>
                  <div className={`flex items-center justify-between border-t pt-3 ${isDark ? "border-white/10" : "border-border/50"}`}>
                    <div>
                      <span className={`text-lg font-bold ${isDark ? "text-amber-400" : "text-primary"}`}>
                        ₹{pkg.price.toLocaleString()}
                      </span>
                      <span className={`text-xs line-through ml-1.5 ${isDark ? "text-white/40" : "text-muted-foreground"}`}>
