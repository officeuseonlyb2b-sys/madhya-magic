...
const ExperienceCard = ({
  ...
  index: number;
  onOpenDetails: (exp: (typeof experiencesData)[0]) => void;
}) => {
          ...
          >
            <div>
              <motion.span
                ...
                className="text-primary font-semibold text-xs uppercase tracking-[0.2em]"
              >
                {exp.subtitle}
              </motion.span>

              <h3 className="text-2xl md:text-3xl lg:text-4xl font-display font-bold text-foreground mt-2 leading-tight">
                {exp.title}
              </h3>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <Link
                to="/contact"
                className="group inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.03]"
              >
                Book Experience
                <ArrowRight
                  size={16}
              ...
              <button
                type="button"
                onClick={() => onOpenDetails(exp)}
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-border/60 text-foreground font-semibold text-sm hover:bg-accent/50 transition-all duration-300"
              >
                Explore Experience
              </button>
            </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-muted-foreground mt-5 max-w-2xl mx-auto text-lg font-body">
            From sacred rituals to wild safaris — discover once-in-a-lifetime
            moments that connect you to the soul of Madhya Pradesh
          </p>
        </motion.div>
